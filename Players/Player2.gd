...
var trashAmount = 0
onready var spriteTrash = $P2/trashSprite
onready var trashCountLabel = $P2/trashCount
onready var x = $P2/x
onready var playerStatus = $P2/singleSprite
onready var segPlasticLabel = $P2/segLabels/segPlastic
onready var segRubberLabel = $P2/segLabels/segRubber
onready var segMetalLabel = $P2/segLabels/segMetal
onready var segLabels = $P2/segLabels
onready var segImages = $P2/threeicons
...
func _physics_process(delta): 
	...
	if enteredTrashArea == true and timeCanGrab == true and grabbingSeg == false and trashLimit != 3 and trashAmount > 0:
		if Input.is_action_just_pressed("p2X"):
			trashLimit += 1
			trashAmount -= 1
			...
			if trashLimit > 1:
				...
				trashHoldGrab.play("Object_0Action.001")
				isTrashYesHold = true
				garbageHold.hide()
			if trashLimit <= 1:
				noHoldGrabSprite.show()
				isTrashNoHold = true
				noHoldGrab.play("Object_0Action.001")
			emit_signal("p2animTimerStart")
	...
	if detector.is_colliding() and item.has_method("metalSeg") and canSeg == true and segLimit != 10:
		if Input.is_action_just_pressed("p2X"):
			...
			if segLimit > 1:
				segGrabSprite.show()
				segGrab.play("Object_0Action.001")
				segHold.hide()
	...
	if detector.is_colliding() and item.has_method("hazard"):
		if isHoldingTrash == true:
			...
			dizzyTrashHold.play("Object_0.003Action_Object_0.003_Object_0.003")
			...
			dizzySegTrashHold.play("Object_0Action_Object_0_Object_0")
			emit_signal("dizzyAnimPlay")
			segHold.hide()
			noHold.hide()
			...
			dizzyRubberHold.play("Object_0Action_Object_0_Object_0")
			emit_signal("dizzyAnimPlay")
			rubberHold.hide()
			noHold.hide()
		else:
	...
	#Movement X
	if Input.is_action_pressed("p2LSRight") and Input.is_action_pressed("p2LSLeft"):
		velocity.x = 0
		isMoving = false
	elif Input.is_action_pressed("p2LSRight"):
		velocity.x = float(speed)
		player.set_rotation_degrees(Vector3(0,90,0))
		isMoving = true
	elif Input.is_action_pressed("p2LSLeft"):
		velocity.x = float(-speed)
		player.set_rotation_degrees(Vector3(0,-90,0))
		isMoving = true
	else :
		...
		isMoving = false
	#Movement Z
	if Input.is_action_pressed("p2LSUp") and Input.is_action_pressed("p2LSDown"):
		velocity.z = 0
		isMoving = false
	elif Input.is_action_pressed("p2LSDown"):
		player.set_rotation_degrees(Vector3(0,0,0))
		velocity.z = float(speed)
		isMoving = true
	elif Input.is_action_pressed("p2LSUp"):
		velocity.z = float(-speed)
...
func _on_Dump_body_entered(body):
	if body.get_name() == "Player2":
		canDump = true
func _on_p2CanMove_timeout():
	speed = currentSpeed
	currentSpeed = speed
#when near metal recycle, allow contact
func _on_metalContainer_p2IsEntered():
	metalContainerInteract = true
func _on_plasticContainer_p2IsEntered():
	plasticContainerInteract = true
func _on_rubberContainer_p2IsEntered():
...
func _on_p2HazardTimer_timeout():
	if isHoldingTrash == true:
