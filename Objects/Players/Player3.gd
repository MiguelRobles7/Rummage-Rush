...
var trashAmount = 0
onready var spriteTrash = $P3/trashSprite
onready var trashCountLabel = $P3/trashCount
onready var x = $P3/x
onready var playerStatus = $P3/singleSprite
onready var segPlasticLabel = $P3/segLabels/segPlastic
onready var segRubberLabel = $P3/segLabels/segRubber
onready var segMetalLabel = $P3/segLabels/segMetal
onready var segLabels = $P3/segLabels
onready var segImages = $P3/threeicons
...
func _physics_process(delta): 
	...
	if enteredTrashArea == true and timeCanGrab == true and grabbingSeg == false and trashLimit != 3 and trashAmount > 0:
		if Input.is_action_just_pressed("p3X"):
			trashLimit += 1
			trashAmount -= 1
	...
	if detector.is_colliding() and item.has_method("hazard"):
		if isHoldingTrash == true:
			...
			dizzySegTrashHold.play("Object_0.003Action_Object_0.003_Object_0.003")
			emit_signal("dizzyAnimPlay")
			segHold.hide()
			noHold.hide()
	...
	#Movement X
	if Input.is_action_pressed("p3LSRight") and Input.is_action_pressed("p3LSLeft"):
		velocity.x = 0
		isMoving = false
	elif Input.is_action_pressed("p3LSRight"):
		velocity.x = float(speed)
		player.set_rotation_degrees(Vector3(0,90,0))
		isMoving = true
	elif Input.is_action_pressed("p3LSLeft"):
		velocity.x = float(-speed)
		player.set_rotation_degrees(Vector3(0,-90,0))
		isMoving = true
	else :
		...
		isMoving = false
	#Movement Z
	if Input.is_action_pressed("p3LSUp") and Input.is_action_pressed("p3LSDown"):
		velocity.z = 0
		isMoving = false
	elif Input.is_action_pressed("p3LSDown"):
		player.set_rotation_degrees(Vector3(0,0,0))
		velocity.z = float(speed)
		isMoving = true
	elif Input.is_action_pressed("p3LSUp"):
		velocity.z = float(-speed)
...
func _on_Dump_body_entered(body):
	if body.get_name() == "Player3":
		canDump = true
func _on_p3CanMove_timeout():
	speed = currentSpeed
	currentSpeed = speed
#when near metal recycle, allow contact
func _on_metalContainer_p3IsEntered():
	metalContainerInteract = true
func _on_plasticContainer_p3IsEntered():
	plasticContainerInteract = true
func _on_rubberContainer_p3IsEntered():
...
func _on_p3HazardTimer_timeout():
	if isHoldingTrash == true:
