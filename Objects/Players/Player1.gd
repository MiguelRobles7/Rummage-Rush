...
func _physics_process(delta): 
	...
	if enteredTrashArea == true and timeCanGrab == true and grabbingSeg == false and trashLimit != 3 and trashAmount > 0:
		if Input.is_action_just_pressed("space"):
			...
			if trashLimit > 1:
				...
				trashHoldGrab.play("Object_0.003Action")
				isTrashYesHold = true
				garbageHold.hide()
			if trashLimit <= 1:
				noHoldGrabSprite.show()
				isTrashNoHold = true
				noHoldGrab.play("Object_0.003Action")
			emit_signal("p1animTimerStart")
	...
	if detector.is_colliding() and item.has_method("metalSeg") and canSeg == true and segLimit != 10:
		if Input.is_action_just_pressed("space"):
			...
			if segLimit > 1:
				segGrabSprite.show()
				segGrab.play("Object_0.003Action")
				segHold.hide()
	...
	if detector.is_colliding() and item.has_method("hazard"):
		if isHoldingTrash == true:
			...
			dizzyTrashHold.play("Object_0Action_Object_0_Object_0")
			...
			dizzySegTrashHold.play("Object_0Action")
			emit_signal("dizzyAnimPlay")
			segHold.hide()
			noHold.hide()
			...
			dizzyMetalHold.play("Object_0Action_Object_0_Object_0")
			emit_signal("dizzyAnimPlay")
			metalHold.hide()
			noHold.hide()
			...
			dizzyPlasticHold.play("Object_0Action_Object_0_Object_0")
			emit_signal("dizzyAnimPlay")
			plasticHold.hide()
			noHold.hide()
...
func _on_p1HazardTimer_timeout():
	if isHoldingTrash == true:
